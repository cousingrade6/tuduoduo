...
public class UrlPictureUpload extends PictureUploadTemplate {
    ...
    private static final List<String> ALLOW_CONTENT_TYPES = Arrays.asList(
    ...
    protected void validPicture(Object inputSource) {
        ...
        // 4. 发送 HEAD 请求验证文件是否存在, try-with-resource 自动释放资源
        try (HttpResponse httpResponse = HttpUtil.createGet(fileUrl)
                .header("Range", "bytes=0-1")  // 只请求前2个字节
                .timeout(2000)//超时，毫秒
                .execute()) {
            // 未正常返回，无需执行其他判断
            if (httpResponse.getStatus() != HttpStatus.HTTP_OK  && httpResponse.getStatus() != HttpStatus.HTTP_PARTIAL) {
                throw new BusinessException(ErrorCode.PARAMS_ERROR , "未找到资源");
            }
            // 5. 文件存在，文件类型校验
            ...
            if (StrUtil.isNotBlank(contentLengthStr)) {
                try {
                    ...
                    int maxSize = 5;
                    ThrowUtils.throwIf(contentLength > maxSize * _1M, ErrorCode.PARAMS_ERROR, "文件大小不能超过 " + maxSize + "MB");
