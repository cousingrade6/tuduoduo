package com.whisky.yupicturebackend.service.impl;
...
import com.whisky.yupicturebackend.manager.JsoupFetcher;
import com.whisky.yupicturebackend.manager.upload.FilePictureUpload;
import com.whisky.yupicturebackend.manager.upload.PictureUploadTemplate;
import com.whisky.yupicturebackend.manager.upload.UrlPictureUpload;
import com.whisky.yupicturebackend.mapper.PictureMapper;
...
import com.whisky.yupicturebackend.utils.ColorSimilarUtils;
import com.whisky.yupicturebackend.utils.ColorTransformUtils;
...
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture>
    ...
    public Integer uploadPictureByBatch(PictureUploadByBatchRequest pictureUploadByBatchRequest, User loginUser) {
        ...
        try {
            log.info("正在获取页面...");
            document = JsoupFetcher.fetchDocument(fetchUrl);
            log.info("获取页面成功");
        } catch (IOException e) {
            log.error("获取页面失败", e);
...
//        checkPictureAuth(loginUser, picture);
    ...
    public void fillReviewParams(Picture picture, User loginUser) {
        if (userService.isAdmin(loginUser)) {
            ...
            picture.setReviewMessage("管理员上传或编辑图片自动过审");
    ...
    public boolean likePicture(Long userId, Long pictureId) {
        // 2. 判断当前登录用户是否点赞过
        String key = "pictures:liked:" + pictureId.toString();
        Boolean isLiked = stringRedisTemplate.opsForSet().isMember(key, userId.toString());
        if(BooleanUtil.isFalse(isLiked)) {
            // 3. 如果未点赞, 可以点赞
            // 3.1 数据库点赞数+1
            boolean likeSuccess = this.update().setSql("likeCount = likeCount + 1")
                    .eq("id", pictureId)
                    .update();
            // 3.2 保存用户Redis的set集合
            if (likeSuccess) {
            ...
            // 4. 如果已经点赞, 取消点赞
            // 4.1 数据库点赞数-1
            boolean cancelSuccess = this.update().setSql("likeCount = likeCount - 1")
                    .eq("id", pictureId)
                    .update();
    ...
    @Override
    public void isPictureLiked(Picture picture, Long userId){
        String key = "pictures:liked:" + picture.getId().toString();
        Boolean isLiked = stringRedisTemplate.opsForSet().isMember(key, userId.toString());
        picture.setIsLiked(BooleanUtil.isTrue(isLiked));
    }
}
